...
def one_step_experiment() -> None:
    monthly: dict = load_datasets.monthly_data()
    rmse_results: list = []
    smape_results: list = []
    writer = pd.ExcelWriter("monthly_model_predictions.xlsx", engine="xlsxwriter")
    ...
    for key, (y_train, y_test) in monthly.items():
        ...
        models: list = ["rf","mlp", "knn", "svr", "arima", "ets", "theta", "tbats", "ces"]
            ...
            rmse_results.append(
                    {
                        ...
                        "rmse_mean": simple_functions.compute_rmse(y_test, all_preds.loc[:,model_name]),
                        "rmse_median": simple_functions.compute_rmse(y_test, all_preds.loc[:,model_name]),
                        "rmse_fi_avg": simple_functions.compute_rmse(y_test, all_preds.loc[:,model_name]),
                        "rmse_stack_svr": simple_functions.compute_rmse(y_test, all_preds.loc[:,model_name]),
                    }
                )
            ...
            smape_results.append(
                    {
                        ...
                        "n_models":  1,
                        "models_used": model_name,
                        "smape_mean": simple_functions.compute_smape(y_test, all_preds.loc[:,model_name]),
                        "smape_median": simple_functions.compute_smape(y_test, all_preds.loc[:,model_name]),
                        "smape_fi_avg": simple_functions.compute_smape(y_test, all_preds.loc[:,model_name]),
                        "smape_stack_svr": simple_functions.compute_smape(y_test, all_preds.loc[:,model_name]),
                    }
                )
        ...
        for num_models in range(len(models), 1, -1):
            ...
            for selected_models in set(combinations(models, num_models)):
                ...
                rmse_mean = simple_functions.compute_rmse(y_test, mean_preds)
                smape_mean = simple_functions.compute_smape(y_test, mean_preds)

                rmse_median = simple_functions.compute_rmse(y_test, median_preds)
                smape_median = simple_functions.compute_smape(y_test, median_preds)

                rmse_fi_avg = simple_functions.compute_rmse(y_test, fi_avg_preds)
                smape_fi_avg = simple_functions.compute_smape(y_test, fi_avg_preds)

                rmse_stack = simple_functions.compute_rmse(y_test, stack_preds)
                smape_stack = simple_functions.compute_smape(y_test, stack_preds)

                rmse_results.append(
                    {
                        "dataset": key,
                        "n_models": "_".join(selected_models).count('_') + 1,
                        "models_used": "_".join(selected_models),
                        "rmse_mean": rmse_mean,
                        "rmse_median": rmse_median,
                        "rmse_fi_avg": rmse_fi_avg,
                        "rmse_stack_svr": rmse_stack,
                    }
                )
    ...
    rmse_results_df.to_csv("monthly_model_sensitivity_rmse_results.csv", index=False)

    smape_results_df = pd.DataFrame(smape_results)
    smape_results_df.to_csv("monthly_model_sensitivity_smape_results.csv", index=False)
